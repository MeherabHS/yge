...
const manifesto =
  "EDUCATE  ·  ORGANIZE  ·  CREATE  ·  ADVOCATE  ·  ACT  ·  LEARN  ·  COLLABORATE  ·  RESTORE  ·  ";

const posters = [
  {
    number: "01",
    title: "Climate Education",
    copy: "Building environmental literacy from primary schools to universities through art, discussion and hands-on learning.",
    href: "/work/little-green-artists",
    className: "poster-education",
    icon: BookOpen,
  },
  {
    number: "02",
    title: "Youth Voice & Advocacy",
    copy: "Amplifying Bangladeshi youth perspectives in local and national climate conversations.",
    href: "/work/youth-in-action-cop29",
    className: "poster-voice",
    icon: Megaphone,
  },
  {
    number: "03",
    title: "Community Action",
    copy: "Grassroots campaigns tackling plastic pollution, waste and ecological restoration across Bangladesh.",
    href: "/work/plastic-awareness-1",
    className: "poster-community",
    icon: Users,
  },
  {
    number: "04",
    title: "Creative Climate Media",
    copy: "Documentaries, photography and storytelling that make environmental crises visible.",
  ...
  {
    number: "05",
    title: "Green Campus Network",
    copy: "Empowering young people to build more sustainable university communities.",
    href: "/work/project-green-campus",
    className: "poster-campus",
    icon: GraduationCap,
  },
];
  ...
  return (
    <svg className="river-contours" viewBox="0 0 520 420" aria-hidden="true">
      {Array.from({ length: 12 }).map((_, index) => (
        <path
          key={index}
          d={`M-60 ${90 + index * 22} C70 ${18 + index * 18}, 105 ${185 + index * 13}, 230 ${100 + index * 20} S410 ${42 + index * 22}, 580 ${120 + index * 17}`}
        />
      ))}
    </svg>
  );
...
function CampaignStamp({ light = false }: { light?: boolean }) {
  return (
    <div
      className={`campaign-stamp ${light ? "campaign-stamp-light" : ""}`}
      aria-label="Youth for a Green Earth campaign mark"
    >
    ...
    <div className="home-editorial">
      <section className="editorial-hero" aria-labelledby="home-title">
        <Image
          ...
          alt="Aerial collage of the Bangladesh river delta and a traditional river boat"
          fill
          priority
          placeholder="blur"
        ...
        <div className="hero-copy">
          <motion.span
            ...
            className="campaign-label label-outline"
          >
            Since 2024
          </motion.span>
          <motion.h1
          ...
          <Link href="/work" className="ink-button">
      ...
      <section className="manifesto-marquee" aria-label="Our manifesto">
        <div className="marquee-track">
          <span>{manifesto.repeat(4)}</span>
          <span aria-hidden="true">{manifesto.repeat(4)}</span>
        </div>
      </section>

      <section
        className="program-spread paper-surface"
      ...
      >
        <div className="program-intro">
          ...
          <Link href="/work" className="text-link">
            View all programs <ArrowRight aria-hidden="true" />
          </Link>
          <svg
            ...
            aria-hidden="true"
          >
            <path d="M7 82c7-41 38-69 78-57 21 7 6 28-10 18-14-9 11-25 44-30M107 5l12 8-13 7" />
          </svg>
        </div>
        <div className="poster-grid">
          {posters.map((poster) => (
            <ProgramPoster key={poster.number} poster={poster} />
          ))}
        </div>
      </section>
      ...
      <section className="eco-feature" aria-labelledby="eco-title">
        ...
        <div className="eco-copy">
          ...
          <h2 id="eco-title">
            Field notes for
            <br />
            <em>a changing climate.</em>
          </h2>
          <p>
            YGE&apos;s publication series brings youth research, environmental
            observation and ideas into public view.
          </p>
          <Link href="/eco-papers" className="lime-button">
            Explore Eco Papers <ArrowRight aria-hidden="true" />
          </Link>
        </div>
        <div className="eco-covers" aria-label="Eco Paper issue index">
          {ecoPapers.map((paper, index) => (
            <motion.article
              key={paper.slug}
              whileHover={{ y: -8 }}
              className={`eco-paper-entry cover-${index + 1}`}
            >
              <Link
              ...
              <Link
                className="eco-issue-link"
                href={`/eco-papers/${paper.slug}`}
              >
                <b>{paper.issueNumber}</b>
      ...
      {story && (
        <section
          className="field-stories paper-surface"
        ...
        >
          ...
          <aside className="field-note" aria-label="Field note">
            ...
            <p>
              Environmental action begins by listening to the people who live
              closest to change.
            </p>
          ...
          <article className="featured-field-story">
            ...
            <div>
              ...
              <h3>{story.title}</h3>
              <p>{story.excerpt}</p>
              <Link href={`/stories/${story.slug}`} className="text-link">
      ...
      <section className="join-strip" aria-labelledby="join-title">
        ...
        <p>
          Be part of a nationwide youth movement for climate justice and a
          greener Bangladesh.
        </p>
