...
const documentaries = [
  {
    ...
    theme: 'River &middot; Pollution &middot; Dhaka',
  },
  {
    id: 'echoes',
    ...
    theme: 'Displacement &middot; Resilience &middot; Kamlapur',
  },
  {
    id: 'breathing',
    title: "Breathing Life Back Into Bangladesh: Let's Clear the Skies Together",
    year: '2024',
    duration: 'Short Documentary',
    description:
    ...
    theme: 'Air Quality &middot; Urban &middot; Youth',
  },
];
      ...
      >
        ...
        <div className="absolute inset-0 p-6 flex flex-col justify-between z-10">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-xs font-bold text-[var(--color-acid-leaf)]">{doc.year}</span>
          </div>

          <div className="space-y-2">
            <p className="font-mono text-[10px] uppercase tracking-widest text-[var(--color-acid-leaf)]" dangerouslySetInnerHTML={{ __html: doc.theme }} />
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
          ...
          >
            <Play size={26} fill="var(--color-forest-ink)" color="var(--color-forest-ink)" aria-hidden="true" />
          </div>
        </div>
      </div>

      <h3
        className="font-display font-700 text-xl leading-tight tracking-tight mb-2 text-[var(--color-paper-white)] group-hover:text-[var(--color-acid-leaf)] transition-colors"
      >
        {doc.title}
      </h3>
      <p
        className="font-mono text-xs mb-3 text-[var(--color-muted-sage)]"
      >
        {doc.duration}
      </p>
      <p
        className="text-sm font-body leading-relaxed text-white/80"
      >
        {doc.description}
      </p>
    </motion.article>
      ...
      <div className="container-yge">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
          <div>
            ...
            >
              Environmental{' '}
              <em className="font-serif italic" style={{ color: 'var(--color-future-violet)' }}>
                Documentaries
              </em>
            </h2>
          </div>
          <p
            className="max-w-xs text-sm font-mono text-[var(--color-muted-sage)]"
          >
            Three films. Three dimensions of Bangladesh&apos;s environmental reality.
          </p>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {documentaries.map((doc, i) => (
            <DocPoster key={doc.id} doc={doc} delay={i * 0.15} isInView={isInView} />
          ))}
        </div>
      </div>
    </section>
