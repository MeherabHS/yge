...
export default function WorkArchivePage({ programs }: { programs: Program[] }) {
  ...
  const [pagination, setPagination] = useState({ key: "", count: 8 });
  ...
  const filters: ProgramFilters = useMemo(
    () => ({
      ...
      sort: (["newest", "oldest", "title"].includes(sortParam ?? "")
        ? sortParam
        : "newest") as WorkSort,
      view: (["list", "grid"].includes(viewParam ?? "")
        ? viewParam
        : "list") as WorkView,
    }),
  ...
  const filteredPrograms = useMemo(
    () => filterAndSortPrograms(programs, filters),
    [programs, filters],
  );
  const paginationKey = [
    pageSize,
  ...
  const updateFilter = (key: keyof ProgramFilters, value: string) => {
    ...
    const isDefault =
      ...
      (key === "view" && value === "list");
    if (!value || isDefault) params.delete(urlKey);
  ...
  return (
    <section
      ...
      id="program-archive"
      aria-labelledby="program-archive-title"
    >
      <div className="work-shell work-archive-intro">
        <div>
          <span className="section-kicker">Program archive</span>
          <h2 id="program-archive-title">
            Ideas become <em>action</em> here.
          </h2>
        </div>
        <div>
          <p>
            Browse YGE’s documented programs across education, advocacy, campus
            life, community engagement, publishing and creative media.
          </p>
          <strong>{String(programs.length).padStart(2, "0")}</strong>
          <span>documented programs</span>
        </div>
      </div>
      <WorkFilters
        filters={filters}
      ...
      <div className="work-shell work-results">
        <p className="work-result-announcement" aria-live="polite">
          Showing {visiblePrograms.length} of {filteredPrograms.length} matching
          programs.
        </p>
        {filters.view === "list" ? (
          <ProgramIndex
            ...
            hasMore={visibleCount < filteredPrograms.length}
            onLoadMore={() =>
              setPagination({
                key: paginationKey,
