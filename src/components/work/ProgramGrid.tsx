...
function ProgramCard({ program, index }: { program: Program; index: number }) {
  return (
    <article className="program-grid-card">
      <div className="program-grid-marker">
        <ProgramMarker program={program} index={index} />
        <span>{program.category}</span>
      </div>
      <div>
        <span
          className={`program-status status-${program.status.toLowerCase()}`}
        >
          {program.status}
        </span>
        <h3>
          <Link href={`/work/${program.slug}`}>{program.title}</Link>
        </h3>
        <p>{program.summary}</p>
      </div>
      <footer>
        <span>
          {programYear(program)} · {program.location}
        </span>
        <Link
  ...
  return (
    <>
      ...
      {hasMore && (
        <button
          type="button"
          className="program-load-more"
          onClick={onLoadMore}
        >
          Load more programs <span aria-hidden="true">↓</span>
        </button>
      )}
    </>
  );
