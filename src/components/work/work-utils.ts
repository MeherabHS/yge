...
export function deriveFilterOptions(programs: Program[]): FilterOptions {
  return {
    ...
    years: [...new Set(programs.map(programYear))].sort((a, b) => b.localeCompare(a)),
    locations: [...new Set(programs.map((program) => program.location))].sort(),
    statuses: [...new Set(programs.map((program) => program.status))].sort(),
  };
}

export function filterAndSortPrograms(programs: Program[], filters: ProgramFilters): Program[] {
  const query = filters.query.trim().toLocaleLowerCase();
  const filtered = programs.filter((program) => {
    const matchesQuery = !query || [program.title, program.shortTitle, program.summary, program.category, program.location].join(' ').toLocaleLowerCase().includes(query);
    return matchesQuery
