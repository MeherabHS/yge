  ...
  return (
    <section className="work-featured" aria-labelledby="featured-event-title">
      ...
      <div className="work-shell">
        <p className="work-featured-label">Featured event <span aria-hidden="true">·</span> 2026</p>
        <div className="work-featured-grid">
          ...
          <motion.article className="featured-event-copy" initial={{ opacity: 0, x: 22 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .25 }}>
            ...
            <dl className="featured-event-meta">
              <div><dt><CalendarDays aria-hidden="true" /> Date</dt><dd>{formatEventDate(event)}</dd></div>
              <div><dt><MapPin aria-hidden="true" /> Venue</dt><dd>{event.venue}, {event.city}</dd></div>
            </dl>
            <div className="featured-event-actions">
              <Link href={`/events/${event.slug}`} className="lime-button">View event details <ArrowRight aria-hidden="true" /></Link>
              <Link href="/events" className="paper-link">Explore all events <ArrowRight aria-hidden="true" /></Link>
            </div>
          </motion.article>
        </div>
      </div>
