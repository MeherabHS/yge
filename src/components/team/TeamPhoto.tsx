...
export default function TeamPhoto({
  ...
  const failed = failedPhoto === member.photo;
    ...
    <div className={`team-photo ${className}`}>
      {showPhoto ? (
        <Image
          ...
          alt={
            member.photoAlt ??
            `${member.name}, ${member.role} at Youth for a Green Earth`
          }
          fill
          priority={priority}
          sizes={sizes}
          onError={() => setFailedPhoto(member.photo)}
          style={{
            objectFit: "cover",
            objectPosition: member.photoPosition ?? "center 24%",
          }}
        />
      ) : (
        <div className="team-photo-fallback">
          <svg viewBox="0 0 220 260" aria-hidden="true">
            ...
            <path
              className="team-photo-leaf"
              d="M110 197c-2-30 7-54 32-75-2 31-11 55-32 75Zm0 0c-13-23-30-37-50-43 6 23 23 39 50 43Zm0 0v31"
            />
          </svg>
