...
export default function EventsClosingCTA({ nextEvent }: { nextEvent?: YGEEvent }) {
  ...
  return (
    <section className="events-closing" aria-labelledby="events-closing-title">
      ...
      <div className="events-shell events-closing-grid">
        <h2 id="events-closing-title">Bring your <em>energy.</em></h2>
        <Link href={href}>{nextEvent ? 'View the next event' : 'Contact YGE'} <ArrowUpRight aria-hidden="true" /></Link>
      </div>
    </section>
  );
}
