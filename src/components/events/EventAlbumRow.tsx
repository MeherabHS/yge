...
export default function EventAlbumRow({ album }: { album: EventAlbum }) {
  return (
    <article className="event-album-row">
      <div
        className="event-album-thumbnails"
        aria-label="Album image placeholders"
      >
        {album.coverImages.slice(0, 3).map((src, index) => (
          <div
            className="event-album-thumbnail"
            key={`${album.id}-${src}-${index}`}
          >
            <EventGalleryImage
              src={src}
      ...
      <div className="event-album-copy">
        ...
        {typeof album.photoCount === "number" && (
          <small>{album.photoCount} photos</small>
        )}
      </div>
      <button
        type="button"
        disabled
        aria-label="Album unavailable until its archive information is verified"
      >
