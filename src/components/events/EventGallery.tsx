      ...
      <div className="events-shell events-moments-layout">
        <header className="events-moments-heading">
          ...
          <EventGalleryFilters
            active={active}
            onChange={setActive}
            categories={eventGalleryCategories}
          />
        </header>
        <div>
          <p className="sr-only" aria-live="polite">
            {visiblePhotos.length} gallery items shown.
          </p>
          {visiblePhotos.length > 0 ? (
            <div className="events-photo-grid">
              {visiblePhotos.map((photo) => (
                <figure
                  className={`events-photo events-photo-${photo.layout}`}
                  key={photo.id}
                >
                  <EventGalleryImage
                    src={photo.src}
                  ...
                  <figcaption>
                    <span>
                      {photo.caption ?? photo.eventTitle ?? "Photo placeholder"}
                    </span>
            ...
            <div className="events-gallery-empty">
              No verified photographs are available for this category yet.
            </div>
          )}
        </div>
      </div>
