...
function formatFeaturedDate(event: YGEEvent) {
  ...
  const month = start.toLocaleDateString('en-GB', { month: 'long' }).toUpperCase();
  const year = start.getFullYear();
  return end ? `${start.getDate()}–${end.getDate()} ${month} ${year}` : `${start.getDate()} ${month} ${year}`;
}
    ...
    <section className="events-featured" aria-labelledby="featured-event-title">
      ...
      <div className="events-shell events-featured-grid">
        ...
        <div className="events-featured-copy">
          <p className="events-featured-meta">Upcoming <span>•</span> {formatFeaturedDate(event)}</p>
          <h2 id="featured-event-title">{event.title}</h2>
          <div className="events-featured-rule" aria-hidden="true"><i /></div>
          {event.theme && (
          ...
          <div className="events-featured-rule" aria-hidden="true"><i /></div>
          {event.motto && <p className="events-featured-motto">{event.motto}</p>}
          <p className="events-featured-location"><MapPin aria-hidden="true" /> {event.venue}, {event.city}</p>
          <Link href={`/events/${event.slug}`}>View event <ArrowUpRight aria-hidden="true" /></Link>
        </div>
      </div>
      <TornPaperEdge />
    </section>
