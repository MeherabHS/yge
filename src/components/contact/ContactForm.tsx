...
export default function ContactForm() {
  ...
  function focusField(field: FieldName) {
    const target = {
      ...
      topic: topicGroupRef.current?.querySelector<HTMLInputElement>('input[type="radio"]') ?? null,
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (contactConfig.formMode === 'mailto') {
      ...
      setSubmitState('idle');
      setSubmitMessage('Your email application should open with the message ready to send.');
      return;
    }

    if (contactConfig.formMode !== 'api' || !contactConfig.endpoint) {
      setSubmitState('error');
      setSubmitMessage(`Online sending is not configured. Please email ${contactConfig.email}.`);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setSubmitState('error');
        setSubmitMessage(result.error ?? `Your message could not be sent. Please email ${contactConfig.email}.`);
        return;
      }
      ...
      setSubmitState('error');
      setSubmitMessage(`Your message could not be sent. Please email ${contactConfig.email}.`);
    }
  }

  const invalidAttempt = submitState === 'error' && Object.keys(errors).length > 0;
  ...
  return (
    <div className="contact-form-sheet">
      ...
      <form id="contact-form" onSubmit={handleSubmit} noValidate aria-label="Contact YGE">
        ...
        <div className="contact-line-field">
          <label htmlFor="contact-organization">Organization / School (optional)</label>
          <input
        ...
        <button className="contact-submit" type="submit" disabled={submitState === 'loading'}>
          {submitState === 'loading' ? (
            <><span>Sending</span><LoaderCircle className="contact-spinner" aria-hidden="true" /></>
          ) : (
            <><span>Send message</span><ArrowRight aria-hidden="true" /></>
          )}
        </button>
        ...
        <AnimatePresence mode="wait">
          {submitMessage && (
            ...
            >
              ...
              <span>{invalidAttempt ? 'Please fill in the required fields before sending.' : submitMessage}</span>
            </motion.div>
          )}
        </AnimatePresence>
      </form>
