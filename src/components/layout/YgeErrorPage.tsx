...
export default function YgeErrorPage({ status, eyebrow, title, message, onRetry, standalone = false }: YgeErrorPageProps) {
  return (
    <div className={`yge-error-shell${standalone ? ' yge-error-standalone' : ''}`}>
      {standalone && (
        <YGELogo priority className="yge-error-emergency-brand" />
      )}

      <section className="yge-error-page" aria-labelledby="error-page-title">
        <div className="container-yge yge-error-container">
          <div className="yge-error-code" aria-hidden="true">{status}</div>
          ...
          <div className="yge-error-copy">
            ...
            <div className="yge-error-actions">
              {onRetry && (
                <button type="button" onClick={onRetry} className="yge-error-primary">
                  <RefreshCw aria-hidden="true" /> Try Again
                </button>
              )}
              <Link href="/" className={onRetry ? 'yge-error-secondary' : 'yge-error-primary'}>
