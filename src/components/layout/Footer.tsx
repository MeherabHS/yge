...
import { footerColumns, primaryCta, primaryNavigation } from '@/content/navigation';
      ...
      <nav className="footer-mobile-primary" aria-label="Footer primary navigation">
        ...
        <ul>
          {mobilePrimaryLinks.slice(4).map((link) => (
            <li key={link.href} className={link.href === primaryCta.href ? 'footer-mobile-contact' : undefined}>
      ...
      <div className="footer-columns">
        ...
        <div className="footer-contact">
          ...
          <div className="footer-socials">
            {Object.entries(siteConfig.social).filter(([, url]) => Boolean(url)).map(([name, url]) => (
              <a key={name} href={url} target="_blank" rel="noreferrer" aria-label={`YGE on ${name}`}><ArrowUpRight aria-hidden="true" />{name}</a>
            ))}
          </div>
        </div>
      </div>
      <div className="footer-meta"><span>© {new Date().getFullYear()} Youth for a Green Earth</span><span>Transforming awareness into action</span></div>
    </footer>
  );
}
