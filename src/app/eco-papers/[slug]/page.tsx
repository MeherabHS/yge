...
export function generateStaticParams() {
  return ecoPapers.map((paper) => ({ slug: paper.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    title: `${paper.title} — YGE Eco Papers`,
    description: paper.summary,
  };
}

export default async function EcoPaperDetail({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    <article className="eco-detail">
      <header className="eco-detail-header">
        <nav aria-label="Breadcrumb">
          <Link href="/eco-papers">
            <ArrowLeft /> Eco Papers archive
          </Link>
          <span>/</span>
          <span>Issue {paper.issueNumber}</span>
        </nav>
        <div className="eco-detail-grid">
          <div className="eco-detail-title">
            <span className="campaign-label">
              Issue {paper.issueNumber} / {paper.category}
            </span>
            <h1>{paper.title}</h1>
            <p>{paper.subtitle}</p>
            <dl>
              ...
              <div>
                ...
                <dd>{paper.publicationDate ?? "To be confirmed"}</dd>
              </div>
              <div>
                <dt>Topics</dt>
                <dd>
          ...
          >
            <Image
              ...
              fill
              priority
              sizes="(min-width: 1024px) 410px, 78vw"
        ...
        <aside>
          {paper.documentUrl ? (
            <a
              ...
              rel="noreferrer"
            >
              <Download /> Read or download PDF
            </a>
          ) : (
            <div className="document-pending">
              <FileClock />
              <strong>Document coming soon</strong>
              <p>
                The publication file has not been released. This link will be
                activated when a verified document is available.
              </p>
            </div>
          )}
      ...
      {related.length > 0 && (
        <section className="related-papers">
          <div>
            <span className="section-kicker lime">Continue reading</span>
            <h2>Related Eco Papers</h2>
          </div>
          {related.map((item) => (
            <Link key={item.slug} href={`/eco-papers/${item.slug}`}>
