...
export const metadata: Metadata = {
  ...
  description:
    "Transparent, evidence-led impact reporting from Youth for a Green Earth activities in Bangladesh.",
};
...
export default function ImpactPage() {
  ...
  return (
    ...
    >
      {/* Hero */}
      <section
        className="section-pad pt-36 pb-20 grain"
      ...
      >
        <div className="container-yge max-w-4xl">
          <span
            ...
            style={{
              backgroundColor: "var(--color-solar-yellow)",
          ...
          <p className="text-body-lg leading-relaxed max-w-2xl text-white/80">
            Youth for a Green Earth does not manufacture impact statistics.
            Every figure below is grounded in publicly documented YGE activities
            across Bangladesh.
          </p>
        </div>
      </section>
      ...
      <section
        ...
        style={{ backgroundColor: "rgba(18,60,47,0.5)" }}
        aria-label="Verified metrics"
      >
        <div className="container-yge">
          <div className="mb-12">
            <span className="font-mono text-xs uppercase tracking-widest text-[var(--color-solar-yellow)] block mb-2">
              Verified Data
            </span>
            ...
            {verified.map((m) => (
              ...
              >
                <div className="space-y-2">
                  ...
                  <h3 className="font-display font-700 text-xl text-[var(--color-paper-white)]">
                    {m.label}
                  </h3>
      ...
      {demo.length > 0 && (
        <section
          ...
          style={{ backgroundColor: "var(--color-forest-ink)" }}
          aria-label="Demonstration figures"
        >
          <div className="container-yge">
            <span className="font-mono text-xs uppercase tracking-widest text-[var(--color-muted-sage)] block mb-2">
              Demo Estimates
            </span>
            <h3 className="font-display font-700 text-2xl text-[var(--color-paper-white)] mb-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {demo.map((m) => (
                <div
                  key={m.id}
                  className="p-6 rounded-2xl border border-dashed border-white/20 opacity-75"
                >
                  <div className="font-display font-800 text-4xl text-[var(--color-solar-yellow)] mb-2">
                    {m.value.toLocaleString()}+
                  </div>
                  <p className="font-display font-700 text-base text-[var(--color-paper-white)] mb-1">
                    {m.label}
                  </p>
                  <p className="text-xs text-[var(--color-muted-sage)]">
                    {m.description}
                  </p>
                </div>
              ))}
        ...
        <div className="container-yge">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            {impactLocations.map((loc) => (
              <div
                key={loc.id}
                className="p-6 rounded-2xl border border-white/10 bg-black/20 space-y-3"
              >
                <span className="font-mono text-xs font-bold text-[var(--color-solar-yellow)]">
                  {loc.region}
                </span>
                <h3 className="font-display font-800 text-2xl text-[var(--color-paper-white)]">
                  {loc.name}
                </h3>
                <div className="flex flex-wrap gap-1.5 pt-2">
                  {loc.activities.map((act) => (
                    <span
                      key={act}
                      className="font-mono text-[10px] px-2.5 py-1 rounded-full bg-white/10 text-white/80"
                    >
                      {act}
                    </span>
                  ))}
